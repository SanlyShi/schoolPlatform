...
<template>
  <div id="dictionary-page" class="container-fluid">
    <div class="row text-center">
      ...
      <v-table :title="translate.tableTitle" :url="url" pagesize="10" :order="true" idField="id" :columns="columns" :multiple="false" :search="true">
        <!--使用插槽-“新增条目”-->
    ...
    <v-modal :show="showAddModal" effect="fade" width="446">
      ...
      <div slot="modal-body" class="modal-body">
        <div class="form-group class-committee-group">
          ...
          <input type="text" class="optionInput" :placeholder="translate.dictionaryNameInputPlaceholder" @click="dicNameDisabled" v-model="dicName" :readonly="flow.rank === 'systemDefined'? true : false" maxlength="15" @change="modifyDicName">
        </div>
        ...
        <div class="form-group" v-show="showOptionInput">
          <input id="formName" type="text" class="form-control name-input" @change="optionInput" maxlength="15" :placeholder="translate.dictionaryOptionInputPlaceholder" v-model="inputContent">
        </div>
        <div class="modal-body-content">
          <!--点击文字变成可编辑状态,拖动排序后optionsArr内元素顺序自动改变-->
          <div v-for="(option,index) in optionsArr" v-dragging="{ item: option, list: optionsArr, group: ' ' }" :key="index">
            <input class="optionInput" type="text" v-model="option.name" @click="editable(option, index)" @change="modifyOption(option,index)" autofocus :readonly="!option.edit" maxlength="20">
            <!-- 选项最右边的icon-->
            <i class="maticon remove" @mousedown="remove(option,index)" v-html="icons('cancel')">cancel</i>
      ...
      <div slot="modal-footer" class="modal-footer">
        <button type="button" class="btn cancel-btn clearBtnBorder" @click="cancel">{{ translate.cancel }}</button>
        <button type="button" class="btn submit-btn clearBtnBorder" @click="submit">{{ translate.submit }}</button>
      </div>
    </v-modal>
...
export default {
  ...
  data: function () {
    return {
      ...
      columns: [],
      // 初始数据,添加table-data
      url: "/table-data/archives/sys/getDictList",
      showAddModal: false,
      flow: {},
      newFlow: {
        id: "",
        dicts: [{ id: "", name: "", value: "", sort: 0 }],
        name: "",
        rank: "customDefined"
      },
      newDictionaryType: {},
            ...
            },
            {
              ...
              callback: function (record, index) {
                if (record.rank !== "systemDefined") {
                  if(confirm('是否确认删除该字典?')){
                    sAjax({
                    // 对于新建的字典类型，后台会自动给一个ID
                    url: `/api/archives/sys/${record.id}/deleteDictType`,
                    dataType: "json",
                    type: 'post',
  ...
  methods: {
    editable: function (option, index) {
      if (this.flow.rank !== "systemDefined") {
        ...
        this.optionInputTemp = option.name;
      } else {
        this.$toast("默认选项无法修改!");
      }
    },
    ...
    submit: function () {
      ...
      if (this.submitCtrl) {
        ...
        if (this.modalType === this.translate.edit) {
          ...
          sAjax({
            url: "/api/archives/sys/updateDictType" ,
            dataType: "json",
            data: {
              id: this.newFlow.id,
          ...
          sAjax({
            url: "/api/archives/sys/saveDictType",
            data: {
              dicts: this.newFlow.dicts,
              name: this.dicName
            },
    ...
    optionInput: function () {
      ...
      if (
        !this.optionsArr.find(item => {
          return item.name === this.inputContent;
        })
      ) {
        // 如果新增的选项无效则不保存
    ...
    modifyOption: function (option, index) {
      ...
      if (
        ...
        this.$toast("该选项已存在!");
        option.name = this.optionInputTemp;
        this.optionInputTemp = "";
      } else {
        if (option.name !== "" && !pattern.test(option.name)) {
          this.newFlow.dicts[index].name = this.optionsArr[index].name;
        }
      }
    },
