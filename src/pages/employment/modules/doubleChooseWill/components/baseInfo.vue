<template>
  <module-container title="基本信息" :inline="inline">
    <el-form v-if="!edit" :model="form" size="small">
      <el-form-item label="双选会名称" v-if="!(rules.name && rules.name.hidden)">
        <p>{{ form.name }}</p>
      </el-form-item>
      <el-form-item label="双选会时间">
        <p>{{form.timeRange.map(time => new Date(time).format('yyyy-MM-dd HH:mm')).join(' ~ ')}}</p>
      </el-form-item>
      <el-form-item label="双选会地点">
        <p>{{ form.address }}</p>
      </el-form-item>
      <el-form-item label="申请时间" v-if="!(rules.applyTime && rules.applyTime.hidden)">
    ...
    <el-form ref="form" :model="form" :rules="formRules" v-else>
      <el-form-item label="双选会名称" prop="name">
        <el-input v-model="form.name" placeholder="请输入双选会名称"></el-input>
      </el-form-item>
      <el-form-item label="双选会时间" prop="timeRange">
        <el-date-picker
      ...
      <el-form-item label="双选会地点" prop="address">
        <el-input v-model="form.address" placeholder="请输入双选会地点"></el-input>
      </el-form-item>
    ...
    data() {
      return {
        formRules: {
          ...
          timeRange: [{ required: true, message: '请选择双选会起止时间', trigger: 'blur' }],
