  ...
  <div id="entryAndExit-page" class="container-fluid" >
    <div class="tab" v-show="!importPage">
       <el-tabs v-model="activeName" type="card" @tab-click='handleClick'>
        <el-tab-pane label="入住管理" name="IN_CHECK_IN"></el-tab-pane>
        <el-tab-pane label="退宿管理" name="IN_RETIREMENT"></el-tab-pane>
      </el-tabs>
    </div>
        ...
        <div slot="btn-group" role="group">
          <div class="btn-group pull-left">
            <div class="batch-options" v-if="tableCheckedList.length > 0">
              ...
              <button  type="button" @click="submit" class="btn btn-bgColor-style clearBtnBorder">{{activeName=='IN_CHECK_IN'?'确认入住':'确认退宿'}}</button>      
            </div>
          </div>
        <div slot="btn-group" class="btn-group pull-right" role="group">
          <!-- <button v-show="activeName=='IN_CHECK_IN'" type="button" class="btn btn-bgColor-style clearBtnBorder" @click="entry()">{{translate.batchEntry}}</button>
          <button v-show="activeName=='IN_RETIREMENT'" type="button" class="btn btn-bgColor-style clearBtnBorder" @click="exit()">{{translate.batchExit}}</button> -->
        </div>
        </div>
...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        tableCheckedList: [],
        activeName: 'IN_CHECK_IN',
        columns: [],
        url: '/table-data/apartment/inandout/allManageRecord?status=IN_CHECK_IN',
        importPage: false,
        importParams: {
          parentPageTitle: '',
          downloadTemplateUrl: '',
    ...
    methods: {
       submit() {
        ...
        if(this.activeName == 'IN_CHECK_IN') {
          url = '/api/apartment/inandout/manageSingleIn'  //批量入住接口
        }else {
          url = '/api/apartment/inandout/manageSingleOut'
        }
        sAjax({
          url: url,
          type: 'post',
          ...
          success: (data) => {
            if(data.state) {
              ...
              this.url = '/table-data/apartment/inandout/allManageRecord?status=' + this.activeName + '&ts=' + new Date().getTime()
            }
          }
        })
      },
      ...
      handleClick() {
        this.tableCheckedList = []
        this.url = '/table-data/apartment/inandout/allManageRecord?status=' + this.activeName
      },
      _initColumns: function () {
        ...
        //   },

          {
            ...
            hidden: false,
            formatter: (record) => {
              var campusText = record.dormitory.campus ? record.dormitory.campus : ''
              return campusText + record.dormitory.zone + (record.dormitory.buildName?record.dormitory.buildName:'') + ' ' + record.dormitory.room + "室" + record.dormitory.bed + "号床"
            }
          },

          {
            ...
            search: {
              type: 'select',
              data: {
                options: [{checkInStatus: 'IN_CHECK_IN',name:'入住确认中'},{checkInStatus: 'IN_RETIREMENT',name: '退宿确认中'}],
                optionsLabel: 'name',
                optionsValue: 'checkInStatus'
              }
            }
          },
            ...
            if(record.checkInStatus != "入住确认中"){
              ...
              callback: (record, index) =>  {
                if (confirm(this.translate.leaveDormitoryConfirmTip)) {
                  sAjax({
                    url: '/api/apartment/inandout/manageSingleOut/' + record.studentId,
                    dataType: 'json',
                    type: 'post',
                    success: (data) => {
                      if (data.state) {
              ...
              callback: (record, index) =>  {
                if (confirm(this.translate.entryDormitoryConfirmTip)) {
                  sAjax({
                    url: '/api/apartment/inandout/manageSingleIn/' + record.studentId,
                    dataType: 'json',
                    type: 'post',
                    success: (data) => {
                      if (data.state) {
      ...
      entry: function () {
        ...
        this.importParams.checkTemplateUrl = '/apartment/inandout/manageBatchIn/excel'
        this.importParams.downloadErrorTemplateUrl = '/apartment/inandout/batchIn/error'
        this.importParams.importBtnType = [{
          url: '/apartment/inandout/manageBatchIn/excel',
          method: 'post',
          isShowBtn: true,
      ...
      exit: function () {
        ...
        this.importParams.checkTemplateUrl = '/apartment/inandout/manageBatchOut/excel'
        this.importParams.downloadErrorTemplateUrl = '/apartment/inandout/batchOut/error'
        this.importParams.importBtnType = [{
          url: '/apartment/inandout/manageBatchOut',
          method: 'post',
          isShowBtn: true,
          name: this.translate.overlayImport,
          success: (data) => {
