  ...
  <div>
    <div id="classManage-page" class="container-fluid" v-if="isParentPage">
      ...
      <div class="class-member-page" v-if="isShowMemberPage">
        <v-schoolmembers :classId="classId" @backClassList="backClassList"></v-schoolmembers>
      </div>
      <!-- 模态框（Modal） -->
...
import VschoolMembers from "../../pages/relationship/schoolMembers.vue";
Vue.component("v-schoolmembers", VschoolMembers);
export default {
  created: function() {
    ...
    sAjax({
      url: "/api/relClass/school/getClassEditSelectOptions",
      dataType: "json",
      type: "get",
    ...
    sAjax({
      url: "/api/relClass/school/getClassManageSelectOptions",
      dataType: "json",
      type: "get",
      success: function(data) {
        if (data.state) {
    ...
    sAjax({
      url: "/api/relClass/school/getDownloadTemplateConditions",
      dataType: "json",
      type: "get",
      success: function(data) {
        if (data.state) {
    ...
    this.importParams.importBtnType = [
      {
        ...
        success: data => {
          if (data.state) {
            this.url =
              "/table-data/relClass/school/getClasses?timestamp=" + new Date();
          }
        }
      }
    ];
  ...
  data: function() {
    return {
      ...
      importParams: {
        ...
        checkTemplateUrl: "/relClass/school/uploadExcel",
        templateName: "",
        importBtnType: null
      },
      isEditClass: false,
  ...
  computed: {
    ...
    downloadUrl: function() {
      return (
        "/relClass/school/downloadTemplate?college=" +
                  ...
                  if (confirm(that.translate.removeConfirmTip)) {
                    sAjax({
                      ...
                      success: function(data) {
                        if (data.state) {
                          that.url =
                            "/table-data/relClass/school/getClasses?timestamp=" +
                            new Date().getTime();
