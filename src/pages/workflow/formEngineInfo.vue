<template>
  <GeminiScrollbar :scrollX="false" id="formEngineInfo">
    <div class="header">
      ...
      <div class="pull-right release-app" :class="releaseAppDisabled?'release-app-disabled':''" v-if="curApp.isPublished" @click="releaseApp('delete')">{{translate.dropOff}}</div>
      <div class="pull-right release-app" :class="releaseAppDisabled?'release-app-disabled':''" v-else @click="releaseApp('release')">{{translate.release}}</div>
      <div class="pull-right release-app" @click="_export">{{translate.export}}</div>
      <div class="title-div">
        ...
        <div class="input-group app-name-div info-value" :style="{ width: inputStyleWidth + 'px'}">
          <input type="text" class="form-control" v-model="curApp.name" :placeholder="translate.applyNamePlaceholder" @input="_limitLength" @change="_saveAppName">
          <div>
            <a href="javascript:;" class="apartment-link">{{curApp.department&&curApp.department.name}}</a>
          </div>
        </div>
        <p class="save-tip text-center" :class="nameChangeMsg.indexOf(translate.error)>=0?'error':''" v-if="nameChangeMsg">{{nameChangeMsg}}</p>
        <div class="form-flow-set" v-if="curRoute === 'formEngine'">
          <span class="info-set" @click="_setAppForm">{{translate.form}}：{{appState.formName|| translate.pleaseSet}}</span>
          <span class="info-connect" v-if="appState.formName"></span>
          <span class="info-set" @click="_setJoinRole" v-if="appState.formName">{{translate.playersSet}}</span>
        </div>
        <div class="app-desc info-value">
          <span class="count-limit">{{curApp.remark?curApp.remark.length:0}}/200</span>
          <textarea class="desc-part" :rows="rowsNumber" maxlength="200" :placeholder="translate.remarkPlaceholder" v-model="curApp.remark" @change="_saveAppDesc"></textarea>
        </div>
        <p class="save-tip text-center" :class="descChangeMsg.indexOf(translate.error)>=0?'error':''" v-if="descChangeMsg">{{descChangeMsg}}</p>
        <div>
          <p>
            <strong>应用直接访问地址：</strong>
            <span style="user-select: text;">{{`http://${host}/${$account.schoolId}/app/${curApp.id}`}}</span>
          </p>
        </div>
      </div>
    ...
    <!-- 第三方链接 -->
    <div class="middle-link-info" v-if="curRoute === 'link'">
      <div class="select-link-manner">
        <div class="radio-item">
          <span class="check" @click="_changeLinkUseBusiness('noUse')">
            <i class="maticon active" v-html="icons('radio_button_checked')" v-if="curApp&&curApp.setting&&!curApp.setting.useBusiness">radio_button_checked</i>
            <i class="maticon" v-html="icons('radio_button_unchecked')" v-else>radio_button_unchecked</i>
          </span>
          <label class="check-label" :class="curApp&&curApp.setting&&!curApp.setting.useBusiness?'active':''">{{translate.directJump}}</label>
        </div>
        <div class="radio-item">
          <span class="check" @click="_changeLinkUseBusiness('use')">
            <i class="maticon active" v-html="icons('radio_button_checked')" v-if="curApp&&curApp.setting&&curApp.setting.useBusiness">radio_button_checked</i>
            <i class="maticon" v-html="icons('radio_button_unchecked')" v-else>radio_button_unchecked</i>
          </span>
          <label class="check-label" :class="curApp&&curApp.setting&&curApp.setting.useBusiness?'active':''">{{translate.useBusiness}}</label>
        </div>
      </div>
      <div class="set-link input-group info-value">
        <input type="text" class="form-control" v-model="curApp&&curApp.setting&&curApp.setting.linkUrl" :placeholder="translate.linkUrlPlaceholder" @change="_saveLink">
      </div>
    </div>
    <div class="middle" v-if="curApp&&curApp.setting&&(curApp.setting.useBusiness!==false)">
      <label class="head-label pull-left">{{translate.businessPeriod}}</label>
      <!-- <label class="head-label pull-left" @click="_openReviseBusiness">{{translate.businessPeriod}}</label> -->
      <a href="javascript:;" class="add-new pull-right" @click="_addNewBusiness">{{translate.addNew}}</a>
      ...
      <GeminiScrollbar class="business-div" id="businessDiv" :scrollY="false">
        <div class="business-scroll">
          <div style="display:inline-block;position:relative;" v-for="(business,bIndex) in businessList" :key="bIndex">
            <div class="mark-div" :class="_compareTime(businessList, bIndex)">
              <span></span>
            </div>
            <div class="business-tab" :title="business.business.name" :class="curBusiness&&business.business.id === curBusiness.business.id?'active':''" @click="_changeBusiness(business,business.business.limitType,bIndex)">
              ...
              <i class="maticon remove-icon" @click.stop="_removeBusiness(bIndex)" v-html="icons('cancel')" v-if="!business.business.isPublic">cancel</i>
            </div>
          </div>
        </div>
      </GeminiScrollbar>
      <div class="info-type-tabs" v-if="curBusiness">
        <div class="info-tab text-center" :class="it.id === appState.infoType?'active':''" @click="_tabClick(it)" v-for="(it, infoIndex) in appState.infoTypes" :key="infoIndex" v-if="(curBusiness&&curBusiness.business.id) || it.id==='base'">{{it.name}}</div>
      </div>
    </div>
    <div class="bottom" v-if="curBusiness&&curApp.setting&&(curApp.setting.useBusiness!==false)">
      <span>
        <a href="javascript:;" class="release-business">
          <span @click="showBus" v-if="!curBusiness.business.isPublic && !curBusiness.business.isPublishing && !curBusiness.business.readyPublish">发布本业务期</span>
          
          <span class="disabled" v-else-if="curBusiness.business.isPublishing && !curBusiness.business.readyPublish">
          ...
          <span @click="_releaseBusiness('stop')" v-else-if="curBusiness.business.isEnable&& !isReviseBusiness && !curBusiness.business.readyPublish">停用本业务期</span>
          <span class="disabled" v-else-if="!curBusiness.business.isEnable && !curBusiness.business.readyPublish">{{translate.businessPeriodDisabled}}</span>
          <span class="disabled" v-else-if="curBusiness.business.readyPublish">即将发布</span>

        </a>
        <!-- <span class="release-business" v-else>
          <span class="disabled">{{translate.businessTip}}~</span>
        </span>-->
      </span>
      <span class="save-business" @click="_cancel()" v-if="curBusiness.business.readyPublish">取消发布</span>
      <!-- 未发布-保存 -->
      <span class="save-business" @click="_save('business')" v-if="!curBusiness.business.isPublic && !curBusiness.business.isPublishing&& !curBusiness.business.readyPublish">{{translate.saveBusinessDeploy}}</span>
      <!-- 已发布-编辑-保存 -->
      <span class="save-business" @click="_openReviseBusiness" v-else-if="curRoute !== 'link'&&curBusiness.business.isEnable && !isReviseBusiness&& !curBusiness.business.isPublishing&& !curBusiness.business.readyPublish">{{translate.editBusinessDeploy}}</span>
      <span class="save-business revise ml0" @click="_reviseBusinessInfo('public')" v-else-if="isReviseBusiness && !curBusiness.business.readyPublish">{{translate.saveBusinessDeploy}}</span>
      <span class="error-tip" :class="errorMsg?'active':''">{{errorMsg}}</span>
      <span v-if="curBusiness.business&&curBusiness.business.id && !curBusiness.business.readyPublish && curBusiness.business.isPublic" class="save-business" style="margin-left:10px">
        <span @click="_updataForm()" v-if="!onLoadingForm">数据同步</span>
        <span v-else style="color:gray;">数据同步中...(<span>{{hadCheckCount}}</span>/<span>{{needCheckCount}}</span>)</span>
      </span>
      <!-- 基本信息 -->
      <div class="info-body container-fluid" v-show="appState.infoType === 'base'">
        <div class="col-sm-6 padding-right">
          <div class="form-group base-item">
            <label>{{translate.businessName}}</label>
            <input type="text" class="form-control valid" :placeholder="translate.businessNamePlaceholder" maxlength="30" v-model="curBusiness.business.name" v-if="!curBusiness.business.isPublic || isReviseBusiness" @change="_changeBusinessName">
            <input type="text" class="form-control" readonly :value="curBusiness.business.name" v-else>
          </div>
        </div>
        <div class="col-sm-6 padding-left">
          <div class="form-group base-item">
            <label>{{translate.schoolYear}}</label>
            <v-select class="info-value btn-block" :options="appState.gradeOptions" :value="curBusiness.business.year||((new Date()).getFullYear()+'_0')" options-value="id" options-label="label" close-on-select :disabled="!curBusiness.business.isPublic || isReviseBusiness ? false:true" search @afterSelected="_afterSelectYear"></v-select>
          </div>
        </div>
        <div class="col-sm-6 padding-right" v-if="curRoute === 'formEngine'">
          <div class="form-group base-item" :class="isReviseBusiness ? 'active' : ''" @click="_reviseBusinessData('form')">
            <label :class="isReviseBusiness ? 'active' : ''">{{translate.form}}({{translate.basicInfoTip}})</label>
            <input class="form-control" :class="isReviseBusiness ? 'active' : ''" readonly :value="appState.formName|| translate.unboundForm">
          </div>
        </div>
        <div class="col-sm-6 padding-left" v-if="curRoute === 'formEngine'">
          <div class="form-group base-item" :class="isReviseBusiness ? 'active' : ''" @click="_reviseBusinessData('players')">
            <label :class="isReviseBusiness ? 'active' : ''">{{translate.playersSet}}({{translate.basicInfoTip}})</label>
            <input class="form-control" :class="isReviseBusiness ? 'active' : ''" readonly>
          </div>
        </div>
      </div>
      <!-- 面向人员 -->
      <div class="info-body container-fluid outer-container" id="faceMember" v-show="appState.infoType === 'faceTo'" v-if="curBusiness.business.id">
        <div class="limit-radios" v-if="!showImportModal&&faceToPeopleType.indexOf(curApp&&curApp.appType&&curApp.appType.userType) < 0">
          ...
          <div class="radio-item small-item" :class="curBusiness.business.limitType === '0'?'active':''">
            ...
            <label class="check-label">{{translate.unlimited}}</label>
          </div>
          <!-- 按条件筛选 -->
          <div class="radio-item large-item" :class="curBusiness.business.limitType === '1'?'active':''">
            <div class="large-check-part">
              ...
              <label class="check-label">{{translate.filterByCondition}}</label>
            </div>
            <div class="large-select-part">
              <div class="select-group">
                <label>{{translate.studentType}}</label>
                <v-select class="info-value btn-block" url="/api/system/dictTypes/1004/dicts" :multiple="true" :value="curBusiness.businessLimit?curBusiness.businessLimit.studentType:''" options-value="value" options-label="label" close-on-select justified :disabled="curBusiness.business.limitType!=='1' || (curBusiness.business.isPublic&&!isReviseBusiness)" @afterSelected="_afterSelectStuType"></v-select>
              </div>
            </div>
            <div class="large-select-part">
              <div class="select-group">
                <label>{{translate.studentGrade}}</label>
                <v-select class="info-value btn-block" :options="appState.gradeOptions2" :multiple="true" :value="curBusiness.businessLimit?curBusiness.businessLimit.grade:''" options-value="id" options-label="label" close-on-select justified :disabled="curBusiness.business.limitType!=='1' || (curBusiness.business.isPublic&&!isReviseBusiness)" @afterSelected="_afterSelectGrade"></v-select>
              </div>
            </div>
            <div class="large-select-part">
              <div class="select-group">
                <label>{{translate.inSchoolStatus}}</label>
                <v-select class="info-value btn-block" :options="appState.inSchoolOptions" :value="curBusiness.businessLimit?parseInt(curBusiness.businessLimit.inSchool):''" options-value="id" options-label="label" close-on-select justified :disabled="curBusiness.business.limitType!=='1' || (curBusiness.business.isPublic&&!isReviseBusiness)" @afterSelected="_afterSelectInSchool"></v-select>
              </div>
            </div>
            <div class="large-select-part">
              <div class="select-group">
                <label>当前状态</label>
                <v-select class="info-value btn-block" :options="schoolStatus" :multiple="true" :value="curBusiness.businessLimit?curBusiness.businessLimit.schoolStatus:''" options-value="value" options-label="label" close-on-select justified :disabled="curBusiness.business.limitType!=='1' || (curBusiness.business.isPublic&&!isReviseBusiness)" @afterSelected="_afterSelectSchoolStatus"></v-select>
              </div>
            </div>
            <div class="large-select-part">
              <div class="select-group">
                <label>培养层次</label>
                <v-select class="info-value btn-block" :options="educationLevel" :multiple="true" :value="curBusiness.businessLimit?curBusiness.businessLimit.educationLevel:''" options-value="value" options-label="label" close-on-select justified :disabled="curBusiness.business.limitType!=='1' || (curBusiness.business.isPublic&&!isReviseBusiness)" @afterSelected="_afterSelectEducationLevelStatus"></v-select>
              </div>
            </div>
            <div v-if="curRoute!=='link'" class="large-select-part">
          ...
          <div class="radio-item small-item" :class="curBusiness.business.limitType === '2'?'active':''">
            ...
            <label class="check-label">{{translate.uploadList}}</label>
            <div class="pull-right opt-div" v-if="curBusiness.business.limitType==='2'">
              <!-- <a href="javascript:;" class="opt-btn" @click="showImportPage">
                <span v-if="true">{{translate.batchUpload}}</span>
                <span v-else>{{translate.againUpload}}</span>
              </a> -->
              <!-- <a href="javascript:;" class="opt-btn" v-if="!curBusiness.business.isPublic" @click="exportUserInfo">{{translate.export}}</a> -->
            </div>
          </div>
          <!-- <div class="stus-list" v-if="curBusiness.business.limitType==='2'">
            ...
            <span class="table-tip" v-else>{{translate.noData}}</span>
          </div> -->
          <!-- 接口对接 -->
          <div class="radio-item large-item" style="height:198px" :class="(curBusiness.business.limitType === '3'&&!curBusiness.business.isPublic)?'active':''">
            <div class="large-check-part">
          ...
          <!-- 人员名单管理 -->
          <div class="stus-list" v-if="(curRoute=='link'&&curBusiness.business.limitType==='2')||(curRoute!=='link'&&curBusiness.business.limitType!==null&&curBusiness.business.limitType!=='3'&&curBusiness.business.limitType!=='4'&&isShowMemberTable)">
            <v-table ref="memberList" title="人员名单管理" :url="allPeopleUrl" pagesize="8" idField="id" :multiple="!curBusiness.business.isPublic || isReviseBusiness" @afterChecked="_afterChecked" :order="true" :search="false" :columns="appState.columns" v-if="curBusiness.business.id">
              ...
              <div slot="btn-group" class="btn-group pull-right" role="group">
                <button v-if="(!curBusiness.business.isPublic || isReviseBusiness)&&checkedNum>0" type="button" class="btn batch-add-btn btn-bgColor-style clearBtnBorder" @click="batchDelete">批量删除</button>
                <button v-if="curRoute!=='link'&&(!curBusiness.business.isPublic || isReviseBusiness)" type="button" class="btn batch-add-btn btn-bgColor-style clearBtnBorder" @click="showAddModal=true">新增</button>
                <button v-else-if="curRoute=='link'&&(!curBusiness.business.isPublic || isReviseBusiness)" type="button" class="btn batch-add-btn btn-bgColor-style clearBtnBorder" @click="showImportPageTwo">批量新增</button>
        ...
        <div class="limit-radios" v-else-if="!showImportModal">
          <div class="radio-item small-item" :class="curBusiness.business.limitType === '0'&&!curBusiness.business.isPublic?'active':''">
            <span class="check" :class="curBusiness.business.isPublic&&!isReviseBusiness?'disable':''" @click="_changeLimitType('0')">
              <i class="maticon" v-html="icons('radio_button_unchecked')" v-if="curBusiness.business.limitType!=='0'">radio_button_unchecked</i>
              <i class="maticon" v-html="icons('radio_button_checked')" v-else>radio_button_checked</i>
            </span>
            <label class="check-label" v-if="curApp&&curApp.appType&&curApp.appType.userType === '3'">{{translate.allFreshmen}}</label>
            <label class="check-label" v-else-if="curApp&&curApp.appType&&curApp.appType.userType === '2'">所有教职工</label>
            <label class="check-label" v-else>{{translate.allInstructor}}</label>
          </div>
          <!-- 按条件筛选 -->
          <div class="radio-item large-item" :class="curBusiness.business.limitType === '1'?'active':''">
            <div class="large-check-part">
          ...
          <div class="radio-item small-item" :class="curBusiness.business.limitType === '2'?'active':''">
            ...
            <!-- <div class="pull-right opt-div" v-if="curBusiness.business.limitType==='2'">
              ...
              <a href="javascript:;" class="opt-btn" @click="exportUserInfo(2)" v-if="!curBusiness.business.isPublic">{{translate.export}}</a>
            </div> -->
          </div>
          <!-- <div class="stus-list" v-if="curBusiness.business.limitType==='2'">
            <v-table :url="peopleUrl" pagesize="5" idField="id" :columns="appState.columns" :order="true" v-if="curBusiness.business.id"></v-table>
          ...
          <!-- 人员名单管理 -->
          <div class="stus-list" v-if="(curRoute=='link'&&curBusiness.business.limitType==='2')||(curRoute!=='link'&&curBusiness.business.limitType!==null&&curBusiness.business.limitType!=='3'&&isShowMemberTable)">
            <v-table ref="memberList" title="人员名单管理" :url="allPeopleUrl" pagesize="8" idField="id" :multiple="!curBusiness.business.isPublic || isReviseBusiness" @afterChecked="_afterChecked" :order="true" :search="false" :columns="appState.columns" v-if="curBusiness.business.id">
              <div slot="btn-group" class="btn-group pull-right" role="group">
                <button v-if="(!curBusiness.business.isPublic || isReviseBusiness)&&checkedNum>0" type="button" class="btn batch-add-btn btn-bgColor-style clearBtnBorder" @click="batchDelete">批量删除</button>
                <button v-if="curRoute!=='link'&&(!curBusiness.business.isPublic || isReviseBusiness)" type="button" class="btn batch-add-btn btn-bgColor-style clearBtnBorder" @click="showAddModal=true">新增</button>
      ...
      <div class="info-body" v-show="appState.infoType === 'timeSet'" v-if="curBusiness.business.id">
        <div class="time-set-tip" v-if="curRoute !== 'link'">在业务期内，学生的分流关系将会实时更新，业务期发布之后业务时间可修改</div>
        <div class="time-set-info-top" v-if="curRoute === 'formEngine'">
          <div class="start-time-group mr12" :class="timeType === 'startTime' ? 'active' : ''" @click="_linkBusinessTime('startTime')">
            <label class="role-name">
              <span class="role-title">业务期</span>
              <span class="time-tip"><span class="time-required">*</span>{{translate.startTime}}</span>
            </label>
            <p class="time-info">{{curBusiness.business.beginTime || translate.noSet}}</p>
          </div>
          <div class="end-time-group mt0" :class="timeType === 'endTime' ? 'active' : ''" @click="_linkBusinessTime('endTime')">
            <label class="role-name">
              <span class="role-title">业务期</span>
              <span class="time-tip"><span class="time-required">*</span>{{translate.endTime}}</span>
            </label>
            <p class="time-info">{{curBusiness.business.endTime || translate.noSet}}</p>
          </div>
        </div>
        <div class="time-set-info" v-if="curRoute === 'formEngine'">
          <div class="time-set-group" v-for="(col,i) in timeSetData" :key="i">
            <div class="start-time-group" :class="timeNodeId === col.nodeId && timeType === 'startDate' ? 'active' : ''" @click="_nodeSelected($event, col.nodeId, 'startDate', col.title)">
              <label class="role-name">
                <span class="role-title">{{col.title}}</span>
                <span class="time-tip">{{translate.startTime}}</span>
              </label>
              <p class="time-info">{{col.startDate || translate.noSet}}</p>
            </div>
            <div class="end-time-group" :class="timeNodeId === col.nodeId && timeType === 'endDate' ? 'active' : ''" @click="_nodeSelected($event, col.nodeId, 'endDate', col.title)">
              <label class="role-name">
                <span class="role-title">{{col.title}}</span>
                <span class="time-tip">{{translate.endTime}}</span>
              </label>
              <p class="time-info">{{col.endDate || translate.noSet}}</p>
            </div>
          </div>
        </div>
        <div class="time-set-info" v-else>
          <div class="start-time-group" :class="timeType === 'startTime' ? 'active' : ''" @click="_linkBusinessTime('startTime')">
            <label class="role-name">
              <span class="time-tip">{{translate.startTime}}</span>
            </label>
            <p class="time-info">{{curBusiness.business.beginTime || translate.noSet}}</p>
          </div>
          <div class="end-time-group" :class="timeType === 'endTime' ? 'active' : ''" @click="_linkBusinessTime('endTime')">
            <label class="role-name">
              <span class="time-tip">{{translate.endTime}}</span>
            </label>
            <p class="time-info">{{curBusiness.business.endTime || translate.noSet}}</p>
    ...
    <v-modal :show="appState.showFormModal" effect="fade" width="374" v-if="!readonly">
      <div slot="modal-header" class="modal-header">
        <h5 class="modal-title">{{translate.selectModal}}</h5>
      </div>
      <div slot="modal-body" class="modal-body">
        <label>{{translate.modalTip}}</label>
        <ul class="list-group">
          ...
          <li class="list-group-item" :class="appState.modelId===form.id?'active':''" v-for="(form, fIndex) in formList" :key="fIndex" @click="appState.modelId = form.id">{{form.name}}{{translate.modal}}</li>
        </ul>
      </div>
      <div slot="modal-footer" class="modal-footer">
        <a href="javascript:;" @click="appState.showFormModal = false">{{translate.cancel}}</a>
        <a class="sure-btn" href="javascript:;" @click="_initWithModel">{{translate.confirm}}</a>
      </div>
    </v-modal>
    <v-modal :show="showIconModal" effect="fade" width="446">
      <div slot="modal-header" class="modal-header iconModal-header">
        <h5 class="modal-title">{{translate.editIcon}}</h5>
      </div>
      <div slot="modal-body" class="modal-body iconModal-body">
        <div class="background-container">
          <span class="background" :style="{ 'background': col.background}" v-for="(col,i) in backgroundList" :key="i" @click="_selectBackground(col)">
            <i class="maticon icon-done" v-if="appState.background === col.background" v-html="icons('done')">done</i>
          </span>
        </div>
        <div class="icon-list">
      ...
      <div slot="modal-footer" class="modal-footer">
        <button type="button" class="btn cancel-btn clearBtnBorder" @click="_cancelPart('icon')">{{translate.cancel}}</button>
        <button type="button" class="btn submit-btn clearBtnBorder" @click="_saveAppIcon">{{translate.confirm}}</button>
      </div>
    </v-modal>
    <v-modal :show="showTableModal" effect="fade" width="748">
      <div slot="modal-header" class="modal-header iconModal-header">
        <h5 class="modal-title">报表设置</h5>
      </div>
      ...
      <div slot="modal-footer" class="modal-footer">
        ...
        <v-modal :show="showBusSettingModal" effect="fade" width="748">
      <div slot="modal-header" class="modal-header iconModal-header">
        <h5 class="modal-title">发布业务期</h5>
      </div>
      <div slot="modal-body" class="modal-body iconModal-body" style="padding:30px 60px">
          <el-radio style="display:block" v-model="busType" label="1">立即发布</el-radio>
          <el-radio v-model="busType" label="2">定时发布</el-radio>
          <div style="padding:0 23px" v-if="busType == '2'">
            <span class="demonstration">发布时间</span>
            <el-date-picker
              v-model="valueTime"
              :picker-options="pickerOptionsValueTime"
      ...
      <div slot="modal-footer" class="modal-footer">
        <button type="button" class="btn cancel-btn clearBtnBorder" @click="showBusSettingModal=false">取消</button>
        <button type="button" class="btn submit-btn clearBtnBorder" @click="_saveRelease">确定</button>
      </div>
    </v-modal>
...
export default {
  ...
  created: function () {
    ...
    this.curRoute = this.curCheckRoutes;
    ...
    this.appState.inSchoolOptions = [
      {
        id: 0,
        label: this.translate.unlimited
      },
      {
        id: 1,
        label: this.translate.inSchool
      },
      {
        id: 2,
        label: this.translate.notAtSchool
      }
    ];
    ...
    this.appState.infoTypes = [
      {
        id: "base",
        name: this.translate.baseInfo
      },
      {
        id: "faceTo",
        name: this.translate.facePeople
      },
      {
        id: "timeSet",
        name: this.translate.timeSet
      },
      {
        id: "description",
    ...
    for (var i = y + 3; i > y - 5; i--) {
      this.appState.gradeOptions.push({
        id: i + "",
        label: i + "-" + (i + 1) + this.translate.schoolYear
      });
    }
    for (var j = y + 3; j > y - 5; j--) {
      this.appState.gradeOptions2.push({
        id: j + "",
        label: j + this.translate.level
      });
    }
    ...
    //   {
    //     id: "inSchoolValue",
    //     title: this.translate.inSchoolStatus,
    //     width: 100,
    //     className: "text-left",
    //     hidden: false,
    //     search: {
    ...
    //         optionsLabel: 'label',
    //         optionsValue: 'id',
    //         options: this.appState.inSchoolOptions,
    //       }
    //     }
    //   },
    //   {
    ...
    //     search: {
    //       type: "select",
    //       url: "/api/system/dictTypes/1004/dicts",
    //       data: {
    //         optionsLabel: 'label',
    //         optionsValue: 'id',
    //         options: [],
  ...
  data: function () {
    return {
      valueTime:'',
      // value2:'',
      pickerOptionsValueTime:{
         disabledDate:(time)=> {
            return time.getTime() < new Date(this.curBusiness.business.beginTime).getTime();
         }
      },
      busType:'1',
      showBusSettingModal:false,
      curBusinessIdx: -1,
      showAddModal: false,
  ...
  computed: {
    ...
    translate: function () {
      return translation(this.lang).formEngineInfo;
    },
    //发布业务期按键是否可用
    ...
    isCache: function () {
      return this.curRoute!=='link'&&(!this.curBusiness.business.isPublic || this.isReviseBusiness);
    },
    cacheKey: function () {
      if(this.curRoute!=='link') {
  ...
  watch: {
    ...
    curBusiness: function () {
      if (this.curBusiness) {
        this._loadBusinessTime("clickBusiness");
        this.getInstancesCode()
      }
    },
    curApp: function () {
      var desc = "";
      ...
      if (this.curApp.appType.userType !== "4") {
        ...
        this.appState.columns = [
          ...
          {
            id: "userNo",
            title: this.translate.instructorNo,
            width: 150,
            className: "text-left",
            hidden: false,
            search: {
  ...
  methods: {
    ...
    _cancel() {
      if (confirm('是否取消发布业务期？取消后，该业务期将不会发布！')) {
        sAjax({
          url: `/api/app/${this.appId}/business/${this.curBusiness.business.id}/publish/stop`,
          dataType: "json",
          type: 'post',
          success: data => {
            if (data.state) {
              this.$toast("已取消发布本业务期");
              this.$nextTick(()=>{
                this.curBusiness.business.readyPublish =false
              })
            } else {
              this.$toast(data.message);
            }
    ...
    showBus(){
      if(this.curBusiness.business.beginTime) {
        this.valueTime = this.curBusiness.business.beginTime
        this.showBusSettingModal = true
      }
      else{
        this.$toast('请设置业务期的开始时间和结束时间')
      }
    },
    _saveRelease(){
      if(this.busType=='1'){
        this._releaseBusiness('release',true)
      }
      if(this.busType=='2'){
        if(this.valueTime){
          this._releaseBusiness('release',false,this.valueTime)
        }
        else{
          this.$toast('定时发布时间未设置！')
        }
      }
    },
    _clearTimeOut() {
    ...
    _saveTableSet() {//保存报表设置
      this.$refs.tableset.save().then(o => {
        if (o.state) {
    ...
    _collegeSearch: function (obj) {
      this.defineSearch.collegeCode = obj.val;
    },
    _stateSearch: function (obj) {
      this.defineSearch.status = obj.val;
    },
    // 点击tab
    ...
    _initPlayersData: function () {
      sAjax({
        url: "/api/apps/" + this.appId + "/settings/players",
        type: "get",
        success: data => {
          if (data.state) {
            if (data.data) {
        ...
        success: (data) => {
          if (data.state) {
            if (data.data.status === 'IMPORT_COMPLETE') {
              ...
              if (businessSetting.business.id == this.curBusiness.business.id) {
                this.curBusiness = businessSetting
                this.$set(this, 'curBusiness', businessSetting)
              }
            }
          } else {
            this.$toast(data.message)
    ...
    _updataForm() {//同步数据
      let url = `api/formEngine/cache/business/${this.curBusiness.business.id}/formInstances`
      request(url).then(o => {
        if (o.state) {
          this.instanceCode = o.data
          this.getInstances()
        }
      })
    },
    getInstancesCode(){
      let url = `api/formEngine/business/${this.curBusiness.business.id}/cache/key`
      request(url).then(o=>{
        if(o.state){
          if(o.data){
            this.instanceCode = o.data.code
            this.getInstances()
            return
          }
        }
        this.onLoadingForm = false
        this.instanceCode = null
      })
    },
    getInstances() {
      this.onLoadingForm = true
      let code = this.instanceCode
      if (!code) {
        this.onLoadingForm = false;
        return
      }
      let url = `api/async/imports/importStatus/${code}`
      sAjax({//不能使用request 不然会有加载中的动画,影响用户操作
        url:url,
        type: "get",
        success: o => {
          if (o.data.status === 'IMPORT_COMPLETE') {//已完成
            localStorage.removeItem('formCode' + this.curBusiness.business.id)
            this.onLoadingForm = false
            this.$toast('同步完成')
          } else if (!o.data.status) {//没有进度,没在同步

          } else if (o.data.status === 'IMPORT_ERROR') {
            this.$toast(o.data.message)
          } else {//同步中
            this.needCheckCount = o.data.count
            this.hadCheckCount = o.data.hadCheckCount
    ...
    // 发布业务期
    _releaseBusiness: function (type,busType,dateTime) {
      if (type === "release") {
        ...
        this.annoreleaseBusinessText = this.translate.announcing;
        // let param = {}
        // if(busType == '1'){
        //   param = {isImmediately:true}
        // }
        // if(busType == '2'){
        //   param = {isImmediately:false,publishTime:dateTtime}
        // }
        let im = this.busType=='1'?'true':'false'
        let dateTimeP = this.busType=='2'?dateTime:''
        this._save("business", () => {
          sAjax({
            url:
              "/api/app/" +
              ...
              "/business/" +
              this.curBusiness.business.id +
              "/publish"+'?isImmediately='+im+'&publishTime='+dateTimeP,
            type: 'post',
            // data:param,
            success: data => {
              ...
              if (data.state) {
                if(this.busType == '2'){
                  this.showBusSettingModal = false
                  this.$toast("业务期将于"+this.valueTime+"发布！");
                  this.$nextTick(()=>{
                    this.curBusiness.business.readyPublish = true
                  })
                }
                ...
                if (data.data.published) {
                  ...
                  this.$toast(this.translate.releaseBusinessSuccess);
                }
              } else {
                if (data.code === '31120') {
                  this.$toast("业务期发布中，请耐心等待");
                    ...
                    sAjax({
                      ...
                      success: data => {
                        ...
                        if (data.state) {
                          ...
                          this._initCheckPublishResultSchedule(this.curBusiness)
                          this.$toast(
                            '应用' + this.translate.release + this.translate.success
                          );
                        } else {
                          this.$toast(data.message);
                        }
    ...
    _removeBusiness: function (index) {
      if (confirm(this.translate.removeBusinessConfirm)) {
    ...
    _stopOrRemove: function (type, index) {
      ...
      if (id) {
        ...
        this.businessList.splice(index, 1);
        this.$toast(this.translate.removeSuccessTip);
      }
    },
    // 添加业务期
    _addNewBusiness: function () {
      ...
      if (this.curRoute === "formEngine") {
        if (
          !(this.curApp.setting.formId && this.curApp.setting.playerSettingId)
        ) {
          this.$toast(this.translate.addNewTip);
          return;
        }
      }
      ...
      // var name =
      //   year + "-" + (year + 1) + this.translate.schoolYear + this.curApp.name;
      var oldName = name;
    ...
    _setAppForm: function () {
      ...
      this.$set(this.appState, "form", this.curApp.form);
      this.appState.showFormModal = true;
    },
    _setJoinRole: function () {
      this.$emit("setJoinRole", {
        businessId: ""
      });
      this._save("", function () { });
    },
    ...
    _afterSelectYear: function (obj) {
      this.curBusiness.business.year = obj.value;
      this.isReviseData = true;
    },
    _afterSelectStuType: function (obj) {
      ...
      this.$set(this.curBusiness.businessLimit, "studentType", obj.value);
    },
    _afterSelectGrade: function (obj) {
      if (!this.curBusiness.businessLimit) {
        this.$set(this.curBusiness, "businessLimit", {});
      }
      this.$set(this.curBusiness.businessLimit, "grade", obj.value);
    },
    ...
    // ==========第三方链接===========
    _changeLinkUseBusiness: function (type) {
      if (type === "use") {
        this.curApp.setting.useBusiness = true;
      } else {
        this.curApp.setting.useBusiness = false;
      }
      sAjax({
        url: "/api/app/" + this.appId + "/useBusiness",
        type: 'post',
        data: {
          useBusiness: this.curApp.setting.useBusiness
        },
        success: data => {
          if (data.state) {
            this.$toast(this.translate.modifySuccess);
          } else {
            this.$toast(data.message);
          }
        }
    ...
    _saveLink: function () {
      sAjax({
        url: "/api/app/" + this.appId + "/url",
        type: 'post',
        data: {
          linkUrl: this.curApp.setting.linkUrl
        },
        success: data => {
          if (data.state) {
            this.$toast(this.translate.modifySuccess);
    ...
    _linkBusinessTime: function (timeType) {
      if (this.curBusiness.business.isPublic && !this.isReviseBusiness) {
        return;
      }
      var ev = window.event;
      ...
      this.isReviseData = true;
      // console.log("this.curSelectedTime",this.curSelectedTime)
      setTimeout(() => {
        this.$date({
          dateFormat: "yyyy-MM-dd hh:ii:ss",
          value: this.curSelectedTime || "",
          type: "init",
          event: ev,
          onSelect: value => {
            this._linkSelectBusinessTime({
              value: value
            });
          }
        });
    ...
    _linkSelectBusinessTime: function (obj) {
      if (obj.value) {
        if (this.timeType === "startTime") {
          if (
            this.curBusiness.business.endTime &&
            this.curBusiness.business.endTime < obj.value
          ) {
            this.errorMsg = this.translate.startTimeTip;
            this.curBusiness.business.beginTime = "";
            return false;
          } else {
            this.curBusiness.business.beginTime = obj.value;
          }
        }
        if (this.timeType === "endTime") {
          if (
            ...
            this.errorMsg = this.translate.endTimeTip;
            this.curBusiness.business.endTime = "";
            return false;
          } else {
            this.curBusiness.business.endTime = obj.value;
    ...
    _nodeSelected: function (event, nodeId, timeType, name) {
      if (this.curBusiness.business.isPublic && !this.isReviseBusiness) {
        return;
      }
      var ev = window.event || event;
      this.timeNodeId = nodeId;
      this.timeType = timeType;
      this.formTimeName = name;
      this.timeSetData.forEach(ele => {
        if (this.timeNodeId === ele.nodeId) {
          this.curSelectedTime = ele[this.timeType];
        }
      });
      this.isReviseData = true;
      setTimeout(() => {
    ...
    _daySelectedOfValue: function (obj) {
      if (obj.value) {
        this.timeSetData.forEach(ele => {
          if (this.timeNodeId === ele.nodeId) {
            if (this.timeType === "startDate") {
              if (ele.endDate && ele.endDate < obj.value) {
                this.errorMsg = this.formTimeName + this.translate.startTimeTip;
                return false;
              }
            }
            if (this.timeType === "endDate") {
              if (ele.startDate && ele.startDate > obj.value) {
                this.errorMsg = this.formTimeName + this.translate.endTimeTip;
                return false;
              }
            }
            this.errorMsg = "";
            ele[this.timeType] = obj.value;
          }
        });
      }
    },
    ...
    _loadBusinessTime: function (type) {
      if (
        ...
        if (this.curRoute === "formEngine") {
          if (this.timeSetData.length === 0 || type === "clickBusiness") {
            this.timeSetData = JSON.parse(
              JSON.stringify(this.curBusiness.business.businessTimeList)
            );
          }
          this.timeSetData.forEach((ele, i) => {
            if (this.players[i]) {
              if (ele.nodeId === this.players[i].id) {
                this.$set(ele, "title", "");
                ele.title = this.players[i].name;
              }
            }
          });
        }
    ...
    _getBusinessTime: function () {
      ...
      this.timeSetData.forEach((ele, i) => {
        if (
          this.curBusiness.business.businessTimeList[i].nodeId === ele.nodeId
        ) {
          this.curBusiness.business.businessTimeList[i].startDate = JSON.parse(
            JSON.stringify(ele.startDate)
          );
          this.curBusiness.business.businessTimeList[i].endDate = JSON.parse(
            JSON.stringify(ele.endDate)
          );
          if (ele.startDate) {
            startTimeData.push(ele.startDate);
            var date = new Date(ele.startDate.replace(/-/g, "/"));
            startChangeTime.push(Date.parse(date));
          }
          if (ele.endDate) {
            endTimeData.push(ele.endDate);
            var date = new Date(ele.endDate.replace(/-/g, "/"));
            endChangeTime.push(Date.parse(date));
          }
        }
      });
      // 业务期时间设置，取节点最早和最晚
      // for (var i in startTimeData) {
      //   var date = new Date(startTimeData[i].replace(/-/g, "/"));
      //   if (Date.parse(date) === Math.min.apply(null, startChangeTime)) {
      //     this.curBusiness.business.beginTime = startTimeData[i];
      //   }
      // }
      // for (var i in endTimeData) {
      //   var date = new Date(endTimeData[i].replace(/-/g, "/"));
      //   if (Date.parse(date) === Math.max.apply(null, endChangeTime)) {
      //     this.curBusiness.business.endTime = endTimeData[i];
      //   }
      // }
      this.curBusiness.business.beginTime = this.curBusiness.business.beginTime
        ? this.curBusiness.business.beginTime
        : "";
      this.curBusiness.business.endTime = this.curBusiness.business.endTime
        ? this.curBusiness.business.endTime
        : "";
    },
    changeBusinessLimit() {
    ...
    _save: function (name, callback) {
      ...
      if (this.curBusiness && 
        ...
        !(this.curBusiness.business.limitType == '1' && this.curRoute == 'link') &&
      ...
      if (this.curBusiness && this.curBusiness.business.limitType == '3') {
      ...
      if (name === "business") {
        if (!this.curBusiness.business.name) {
          this.$toast(this.translate.nameError);
          this.releaseBusinessDisabled = false;
          return false;
        }
        ...
        if (method === "put") {
          if (this.curRoute === "formEngine") {
            url +=
              ...
              "&playerSettingId=" +
              this.curBusiness.playerSettingId +
              "&key=" +
              this.cacheKey;
          }
        }
        if (this.curRoute === "formEngine") {
          ...
          this.curBusiness.business.beginTime = this.curBusiness.business
            .beginTime
            ? this.curBusiness.business.beginTime
            : "";
          this.curBusiness.business.endTime = this.curBusiness.business.endTime
        ...
        if (this.curBusiness && this.curBusiness.business.businessTimeList.length > 0) {
          if (!this.curBusiness.business.endTime || !this.curBusiness.business.beginTime) {
            this.$toast("请设置业务期的开始时间和结束时间")
            this.releaseBusinessDisabled = false;
            return false;
          }
        ...
        request(url, curBusiness, 'post').then(data => {
          if (data.state) {
            ...
            if (callback) {
              ...
              this.$toast(this.translate.saveBusinessSuccess);
    ...
    releaseApp: function (type) {
      if (this.releaseAppDisabled) {
        return false;
      }
      this.releaseAppDisabled = true;
      var tip = "";
      var successTip = "";
      var method = "";
      var url = "/api/app/" + this.appId + "/public";
      if (type === "release") {
        tip = this.translate.releaseTip;
        successTip = this.translate.release;
        method = "post";
      } else {
        tip = this.translate.dropOffTip;
        successTip = this.translate.dropOff;
        method = "delete";
        url = "/api/app/" + this.appId + "/deletePublic";
      }
      if (confirm(tip)) {
        sAjax({
          url: url,
          type: 'post',
          success: data => {
            this.releaseAppDisabled = false;
            if (data.state) {
              this.$toast(successTip + this.translate.success);
              this.curApp.isPublished = !this.curApp.isPublished;
            } else {
              this.$toast(data.message);
            }
    ...
    _saveAppName: function () {
      if (!this.curApp.name) {
        this.nameChangeMsg = this.translate.nameError;
        return false;
      }
      sAjax({
        ...
        success: data => {
          if (data.state) {
            this.$emit("update", this.curApp);
            this.nameChangeMsg = this.translate.modifySuccess;
            setTimeout(() => {
              this.nameChangeMsg = "";
            }, 2000);
          } else {
            this.nameChangeMsg = this.translate.error + "：" + data.message;
          }
        }
      });
    },
    _saveAppDesc: function () {
      sAjax({
        ...
        success: data => {
          if (data.state) {
            this.$emit("update", this.curApp);
            this.descChangeMsg = this.translate.modifySuccess;
            setTimeout(() => {
              this.descChangeMsg = "";
            }, 2000);
          } else {
            this.descChangeMsg = this.translate.error + "：" + data.message;
          }
        }
      });
    },
    ...
    _reviseBusinessInfo: function (type = '') {
      ...
      var parameter = {
        ...
        beginTime: this.curBusiness.business.beginTime,
        endTime: this.curBusiness.business.endTime,
        key: this.cacheKey,
...
#formEngineInfo {
  ...
  .bottom {
    ...
    .end-time-group.active,
    .start-time-group.active {
