<template>
      ...
      <div class="container-fluid">
        <div class="row">
          <div class="col-sm-9 croppa-parent">
            <croppa class="croppa" :placeholder="'选  图'" :quality="1" :placeholder-font-size="48" :zoom-speed="3" :reverse-scroll-to-zoom="false" v-model="croppa" :width="width" :height="height" :accept="acceptType" :file-size-limit="fileSizeLimit" :show-remove-button="false" :disable-click-to-choose="false" @new-image-drawn="newImage" v-if="show">
              <img slot="initial" :src="imageUrl" />
            </croppa>
            <span class="maticon remove-image" v-html="icons('cancel')" @click="_remove()"></span>
          </div>
          <div class="col-sm-3">
  ...
  <div v-else-if="show">
    <input type="file" v-if="canNotGetFromAlbum" capture="camera" id="imgUploads" style="display:none;" accept="image/*" @change="onChangeImg($event, 2)">
    <input type="file" v-else id="imgUploads" style="display:none;" accept="image/*" @change="onChangeImg($event, 2)">
    <MCropper :imgUrl="imageUrl" :showCropper="showCropper" @cancel="_cancel" @save=saveImg></MCropper>
  </div>

</template>
<script>
import MCropper from "../../../../mobile/components/MCropper.vue";
export default {
  data: function () {
    return {
      croppa: null,
  ...
  methods: {
    ...
    onChangeImg(e, num) { //从相册选取文件
      var file = e.target.files[0]
      let str = "(.*)\.("+this.acceptType.replace(/,/g,'|').replace(/image\//g,'')+")$"
      let reg  = new RegExp(str)
      if(!e.target.value){
        this.show=false
      }
      if (!reg.test(e.target.value)) {
        this.$toast('图片类型必须是'+this.acceptType.replace(/image\//g,'')+'中的一种');
        this.show=false
      }
  ...
  mounted: function () {
    ...
    this.$nextTick(() => {
      $('.croppa canvas').css({
        width: this.width * 300 / maxV,
        height: this.height * 300 / maxV
      })
      this._getImageUrl()
      if (this.isMobile) {
