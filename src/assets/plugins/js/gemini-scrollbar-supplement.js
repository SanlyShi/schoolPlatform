...
(function () {
  ...
  GeminiScrollbar.prototype.create = function create () {
    ...
    this._scrollbarVerticalElement.style.display = this.scrollY ? '' : 'none'
    this._scrollbarHorizontalElement.style.display = this.scrollX ? '' : 'none'
  ...
  GeminiScrollbar.prototype.update = function update () {
    ...
    // 补充横纵向开关逻辑
    this._viewElement.style.width = ((this.element.offsetWidth + (this.scrollY ? SCROLLBAR_WIDTH : 0)).toString() + 'px')
    this._viewElement.style.height = ((this.element.offsetHeight + (this.scrollX ? SCROLLBAR_WIDTH : 0)).toString() + 'px')
    if (!this.scrollX) {
      this._viewElement.style.overflowX = 'hidden'
    }
    if (!this.scrollY) {
      this._viewElement.style.overflowY = 'hidden'
    }
...
export default window.GeminiScrollbar
