<template>
  <div>
    <!-- loading -->
    <div class="empty-categories loader" v-if="loading" v-html="loader.iconWithText">
    </div>
    <div class="empty-categories loader" v-else-if="!hasAuthority">
      您无管理签到项目的权限
    </div>
    <div class="empty-categories" v-else-if="!categories.length">
      {{ '暂无项目分类' }}
    </div>
    <el-collapse :value="activeCategoryId" @change="selectCategory" accordion v-else>
      <el-collapse-item class="i-sign-category-item" v-for="(category, index) in categories" :name="category.id" :key="index" :itemWrapCorrectionHeight="48">
        <template slot="title">
          <div class="category-item-title" @click.stop="toEditCategoryName(category)" v-if="isManager">{{category.name}}
        ...
        <template slot="content">
          <div class="empty-projects" v-if="!projects[category.id]">
            nothing here
          </div>
          <div class="empty-projects loader" v-else-if="projects[category.id].loading" v-html="loader.iconWithText"></div>
          <div class="empty-projects" v-else-if="!projects[category.id].list.length">
            暂无签到项目
          </div>
          <el-collapse :value="activeProjectId" @change="selectProject" accordion v-else>
            <el-collapse-item class="i-sign-project-item" v-for="(project, index) in projects[category.id].list" :name="getProjectCollapseItemId(category, project)" :key="index">
              <template slot="title">
                <div class="project-name-container">
                  <div class="project-name">{{project.name}}</div>
                  <div class="project-edit" @click.stop="toEditProjectName(project)" v-if="isManager">
                    <v-icon name="edit" :title="i18n.editName"></v-icon>
                  </div>
                </div>

                <div class="project-actions" v-if="isManager">
                  <div class="people-restration">
                    <button type="button" class="btn btn-primary clearBtnBorder" @click.stop=";">
                      ...
                      <span v-html="project.peopleLoading ? (loader.small) : getLimitPeopleDesc(project)"></span>
                    </button>
                    <transition name="fade">
                      <ul class="list-popper">
                        ...
                        <li @click.stop="setLimitType(project, 'ALL_SCHOOL')">全校在校生</li>
                        <li @click.stop="toImport(project)">自定义名单导入</li>
                        <li @click.stop="showSearch(project)">高级搜索</li>
                        <!-- <li @click.stop="toImport(project)">排除人员名单</li> -->
                      </ul>
                    </transition>
                  </div>
                  <button type="button" class="btn btn-primary clearBtnBorder" @click.stop="toProxySettings(project)">
                    <v-icon class="pull-left" name="settings"></v-icon>{{i18n.proxySettings}}
                  </button>
                  <button type="button" class="btn btn-primary clearBtnBorder" @click.stop="toAddSign(project)">
                    <v-icon class="pull-left" name="add"></v-icon>{{i18n.addSign}}
                  </button>
                </div>
              ...
              <template slot="content">
                <div class="signs">
                  <div v-if="!project.signs">
                    nothing here
                  </div>
                  <div class="empty-signs loader" v-else-if="project.signs.loading" v-html="loader.iconWithText"></div>
                  <div class="empty-signs" v-else-if="!project.signs.list.length">
                    {{ isManager ? '该项目下没有签到' : '暂无签到' }}
                  </div>
                  <div class="sign" v-for="(sign, index) in project.signs.list" :class="{ 'sign-hover': !isMine, 'sign-success': sign.signInStatus == 'SIGN' || sign.signInStatus == 'MANUAL_ENTRY', 'sign-failed': sign.signInStatus == 'TIME_OUT_NOT_SIGN', 'sign-without-icon': isProxy }" :key="index" @click="toSignInfo(project, sign)" v-else>
                    {{sign.name}}
                    <v-icon name="delete" class="sign-status-icon delete-sign" v-if="isManager" @click.native.stop="deleteSign(project, sign)"></v-icon>
                    <v-icon name="done" class="sign-status-icon" v-if="isMine && (sign.signInStatus == 'SIGN' || sign.signInStatus == 'MANUAL_ENTRY')"></v-icon>
                    <v-icon name="clear" class="sign-status-icon" v-else-if="isMine && (sign.signInStatus == 'TIME_OUT_NOT_SIGN')"></v-icon>
                  </div>
                </div>
              </template>
            </el-collapse-item>
            <div class="show-more" v-if="projects[category.id].hasMore && !projects[category.id].pageLoading" @click="showMore(category)">查看更多</div>
            <div class="page-loading loader" v-if="projects[category.id].pageLoading" v-html="loader.iconWithText"></div>
    ...
    <v-modal :show="peopleConditionsModal.show" effect="fade" width="500">
      <div slot="modal-header" class="modal-header">
        <h5 class="modal-title">
          {{peopleConditionsModal.title}}
        </h5>
      </div>
      <div slot="modal-body" class="modal-body container-fluid">
        <div class="conditions">
          <ul>
            <li>
              <label>培养层次</label>
              <v-select ref="educationLevel" :options="limitOptions.educationLevel" v-model="selectLimitOptions.educationLevel" options-value="id" options-label="text" :required="true" @afterSelected="selectEducationLevel"></v-select>
            </li>
            <li>
              <label>年级/label>
              <v-select ref="grade" :options="peopleConditionsModal.grades" v-model="selectLimitOptions.grade" options-value="id" options-label="text" :required="true" @afterSelected="selectGrade"></v-select>
            </li>
            <li>
              <label>在校状态</label>
              <v-select ref="inSchoolStatus" :options="peopleConditionsModal.inSchoolStatusOptions" v-model="selectLimitOptions.inSchoolStatus" options-value="id" options-label="text" :required="true" @afterSelected="selectInSchoolStatus"></v-select>
            </li>
            <li>
              <label>学生当前状态</label>
              <v-select :options="peopleConditionsModal.currentSchoolStatusOptions" v-model="selectLimitOptions.currentSchoolStatus" options-value="id" options-label="text" :required="true" @afterSelected="selectInSchoolStatus"></v-select>
            </li>
      ...
      <div slot="modal-footer" class="modal-footer">
        <button type="button" class="btn cancel-btn clearBtnBorder" @click="peopleConditionsModal.show = false">{{i18n.cancel}}</button>
        <button type="button" class="btn submit-btn clearBtnBorder" @click="setConditionLimit(peopleConditionsModal.project)">{{i18n.sure}}</button>
      </div>
    </v-modal>
  </div>
</template>
...
<script>
import ELCollapse from './collapse/collapse'
import ELCollapseItem from './collapse/collapseItem'
...
export default {
  ...
  data: function () {
    return {
      ...
      peopleConditionsModal: {
        ...
        gradesMax: 10,
        currentSchoolStatusOptions: [{ id: 0, text: '本人在校' }, { id: 'IN_SCHOOL', text: '请假' }, { id: 'NOT_IN_SCHOOL', text: '外出实习' }, { id: 'NOT_IN_SCHOOL', text: '其他' }],
        inSchoolStatusOptions: [{ id: 0, text: '不限制' }, { id: 'IN_SCHOOL', text: '在校' }, { id: 'NOT_IN_SCHOOL', text: '不在校' }]
      },
      selectLimitOptions: {
        educationLevel: 0,
        grade: 0,
  ...
  methods: {
    ...
    toSelectCondition: function (project) {
      this.selectLimitOptions.educationLevel = 0
      this.selectLimitOptions.grade = 0
      this.selectLimitOptions.inSchoolStatus = 0
      // this.$refs.educationLevel.selectedItems = [{ id: 0, text: '不限制' }]
      // this.$refs.grade.selectedItems = [{ id: 0, text: '不限制' }]
      // this.$refs.inSchoolStatus.selectedItems = [{ id: 0, text: '不限制' }]
    ...
    selectEducationLevel: function (obj) {
      ...
      this.$nextTick(function () {
        this.peopleConditionsModal.educationLevelLabel = obj.target && obj.target.selected || 'obj.target.selected is null'
      })
    },
    ...
    selectInSchoolStatus: function (obj) {
      ...
      this.$nextTick(function () {
        this.peopleConditionsModal.inSchoolStatusLabel = obj.target && obj.target.selected || 'obj.target.selected is null'
      })
    },
      ...
      switch (project.limitListOfPeople) {
        ...
        case 'BY_CONDITIONS':
          ...
          if (project.inSchoolStatus) {
            conditions.push(project.inSchoolStatus == 'IN_SCHOOL' ? '在校' : '不在校')
          }
