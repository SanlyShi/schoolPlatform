    ...
    <div class="em-calendar__main">
      <div class="em-calendar__panel">
        <div class="em-calendar__panel-header">
          <div
            class="em-calendar__btn em-calendar__btn-left"
            @click="switchPrevYear()">
            <i class="el-icon-d-arrow-left"></i>
          </div>
          <div
            class="em-calendar__btn em-calendar__btn-left"
            @click="switchPrevMonth()">
            <i class="el-icon-arrow-left"></i>
          </div>
          <span>{{currentMonth.year}} 年</span>
          <span>{{currentMonth.month + 1}} 月</span>
          <div
            class="em-calendar__btn em-calendar__btn-right"
            @click="switchNextYear()">
        ...
        <div class="em-calendar__panel-content" v-if="!loadingRecruitment">
          <table class="em-calendar__table">
            ...
            <tr class="em-calendar__tr" v-for="row in 6" v-bind:key="row">
              <td class="em-calendar__td" v-for="col in 7" v-bind:key="col">
                <el-popover
                  placement="top"
                  ...
                  <div>
                    <div
                      v-for="(info, index) in recruitmentInfo[getDateString(currentMonth.dayList[(row-1)*7+col-1])]"
                      :key="index">
                      <el-button type="text" :class="{ 'is-dcw': info.type === 'DOUBLE_CHOOSE' }" @click="showRecruitment(info)">{{info.startTime + ' ' + info.name}}</el-button>
                    </div>
                  </div>
                  <div
                    v-if="currentMonth.dayList[(row-1)*7+col-1].isCurrMonth !== 0"
                    class="otherMonth"
                    :class="{ 'has-events': recruitmentInfo[getDateString(currentMonth.dayList[(row-1)*7+col-1])] }"
                    slot="reference">
                    {{ currentMonth.dayList[(row-1)*7+col-1].value}}
                  </div>
                  <div
                    v-else-if="currentMonth.dayList[(row-1)*7+col-1].isCurrMonth === 0"
                    :class="{
                      today: isToday(currentMonth.dayList[(row-1)*7+col-1]),
                      'has-events': recruitmentInfo[getDateString(currentMonth.dayList[(row-1)*7+col-1])]
                    }"
                    slot="reference">
                    {{ currentMonth.dayList[(row-1)*7+col-1].value}}
                  </div>
  ...
  export default {
    ...
    methods: {
      ...
      getCurrentMonth(year, month) {
        ...
        this.loadRecruitmentInfoByMonth(year, month)
      },
      async loadRecruitmentInfoByMonth(year, month) {
        this.loadingRecruitment = true
        for (let i = 0; i <= 2; ++i) { // 上月，本月，下月
          let normalizedMonth = month + i
          if (this.recruitmentInfoMonthInit[normalizedMonth]) { // 该月已经初始化过了
            continue
          }
          const recruitments = (await this.getRecruitmentsByYearMonth(year, normalizedMonth))
                                  .filter(recruitment => ['PREACH_MEETING', 'DOUBLE_CHOOSE'].includes(recruitment.type))
          const insertedDates = []
          for (const recruitment of recruitments) {
            const [date, time] = recruitment.beginTime.split(' ')
            this.recruitmentInfo[date] = this.recruitmentInfo[date] || []
            this.recruitmentInfo[date].push({
              id: recruitment.id,
          ...
          for (const date of insertedDates) {
            this.recruitmentInfo[date].sort((a, b) => {
              return a.startTime === b.startTime ? 0 : (a.startTime > b.startTime ? 1: -1)
            })
          }
          this.recruitmentInfoMonthInit[normalizedMonth] = true
        }
        this.loadingRecruitment = false
      },
      async getRecruitmentsByYearMonth(year, month) {
        const { state, message, data } = await pAjax({
          url: `/api/employment/platform/recruitmentCalendar?time=${year}-${month > 9 ? month : '0' + month }-01&isDate=0`
        })
        if (!state) {
          this.$toast(message)
        }
      ...
      showRecruitment(info) {
        switch (info.type) {
          case 'PREACH_MEETING':
            this.$emit('toSomePage', { path: '/student/techin/detail', props: { id: info.id,from: 'index' }});
            break
          case 'DOUBLE_CHOOSE':
            this.$emit('toSomePage', { path: '/student/doubleChooseWill/detail', props: { id: info.id, name: info.name }});
            break
        }
      }
    },
