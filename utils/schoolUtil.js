...
module.exports = {
  ...
  getSchoolLoginConfig: (schoolUid, callback) => {
    getSchoolConfig(schoolUid, (err, config) => {
      ...
      callback(null, config.login, config);
    })
  },
...
let getSchoolConfig = (schoolUid, callback) => {
  ...
  //     });
      //下面是从后端接口取配置存在redis上（使用中）
      if(!schoolUid){
        schoolUid = ''
      }
      let option = {
        url: `/official/schools/config`,
        body:{schoolUid},
        method: 'get',
        json: true,
        headers: {
      ...
      backendUtil.requestBackend(option, function (error, response, body) {
        ...
        redisUtil.set(getSchoolConfigCacheKey(body.data.schoolUid), JSON.stringify(body.data));
        redisUtil.set(key, JSON.stringify(body.data));
      })
    }
  })
};
