...
module.exports = {
  getAccessToken: (verifyRequest, ybUid, callback, specifyAppId, specifyAppSecret) => {
    let oauthInfo;
    let result = {
      accessToken: null,
      expireIn: Infinity,
    ...
    try {
      oauthInfo = decodeVerifyRequest(verifyRequest, specifyAppId, specifyAppSecret);
      oauthInfo = JSON.parse(oauthInfo);
    } catch (err) {

      getTokenInfo(ybUid, specifyAppId || appId, (err, tokenInfo) => {
        if(err) {
          return callback(err);
        }
  ...
  revokeToken: (accessToken, callback, specifyAppId) => {
    request.post(`https://openapi.yiban.cn/oauth/revoke_token`, {
      form: {
        client_id: specifyAppId || appId,
        access_token: accessToken
      }
    }, resDefaultHandler.bind({ callback }));
  },
  getOauthUrl: (specifyAppId, specifyAppSite) => {
    return `https://oauth.yiban.cn/code/html?client_id=${ specifyAppId || appId }&redirect_uri=${ specifyAppSite || appSite }`
  }
};

let getTokenInfo = (ybUid, appId, callback) => {
...
let decodeVerifyRequest = (data, appId, appSecret) => {
  let key = appSecret || conf.dev.yiban.appSecret;
  let iv = appId || conf.dev.yiban.appId;
