...
router.all('/login/outLogin', schoolIdAuth)

// 录取查询页面
router.all('/enroll/enrollSearch', schoolIdAuth)
router.all('/messageBox/MMessageDetail', schoolIdAuth)
// 招生计划页面
...
// 在线咨询页面
router.all('/enroll/onlineConsultation', schoolIdAuth)
...
// const outLoginAuth = (req, res, next) => {
//   loginUtil.getConfig(req.schoolId, (err, loginConfig) => {
//     if (err) {
//       return res.jsonTip(err)
//     }
//     if (!loginConfig.outLogin) {
//       return res.jsonTip(new Error('校外报名登录未启用'))
//     }
//     next()
//   })
// }
// router.post('/certification/outLogin', outLoginAuth)
...
const testLoginAuth = (req, res, next) => {
  loginUtil.getConfig(req.params.id, (err, loginConfig) => {
    if (err) {
      return res.jsonTip(err)
    }
