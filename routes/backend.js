...
//获取系统授权
router.get('/api/official/schools/empower/config', (req, res) => {
  let schoolId = req.querybody.schoolId
  schoolUtil.getConfig(schoolId, function (error, data) {
    if (error) {
      return res.json({
        message: error,
        state: false
    ...
    let ret = {
      data:data?data.empower:{},
      code:backendCode.success,
      message:'success'
    }
    res.json(ret)
  })
...
router.get('/schoolcustom/qrCode', (req, res) => {
  let option = {
    url: '/schoolcustom/qrCode',
    method: 'get',
    json: true,
    headers: {
      'content-type': 'application/json',
...
//  console.log("pageIndex",pageIndex)
  ...
  backendUtil.requestBackend(options, function (error, response, body) {
    ...
    if (body && body.code === backendCode.success) {
      ret = {
        state: true,
        total: body.data ? body.data.totalElements?body.data.totalElements:body.data.totalElement : 0
      }
...
router.all('/s/:schoolId/:code', (req, res, next) => {
  ...
  backendUtil.requestBackend(options, function (error, response, body) {
    ...
    if (body && body.code === backendCode.success) {
      ...
      res.setHeader('Set-Cookie', 'sk=' + encryptStr + ';path=/; HttpOnly')
      if (isCheckLogin) {
